...
def _default_get_verifier_callback():
...
class FatSecretClient(object):
    ...
    class _MethodProxy(object):
        ...
        def __call__(self, *args, **kwargs):
            return self.__client.request(self.__path, *args, **kwargs)

    def __getattr__(self, name):
        return self.__dict__.get(name, FatSecretClient._MethodProxy(self, name))
    
    def authorize(self, key=None, callback=_default_get_verifier_callback):
        if not self.loadToken(key):
            token = self.getRequestToken()
            verifier = self.getUserAuthentication(token, callback)
    ...
    def getUserAuthentication(self, token, callback=_default_get_verifier_callback):
        import webbrowser
